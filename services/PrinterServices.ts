...
// Extend existing Navigator interface without conflicting with existing usb declaration
declare global {
  interface Navigator {
    serial: any; // Web Serial API
...
class PrinterService {
  ...
  private activeDevice: USBDevice | any = null;
  ...
  public async connectUsbPrinter(): Promise<boolean> {
    ...
    try {
      // Cast navigator to access usb property
      ...
      const device = await navigatorUsb.requestDevice({
        filters: [
          { vendorId: 0x0FE6, productId: 0x811E } // Common thermal printer vendor/product IDs
        ]
      });
  ...
  private findOutEndpoint(device: USBDevice): number | null {
